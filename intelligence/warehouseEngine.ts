 ...
 */
function sanitizeAvailableQuantity(value: number): number {
...
function getAvailableQuantity(
  ...
  return inventory
    ...
    .reduce(
      (sum, record) => sum + sanitizeAvailableQuantity(record.availableQuantity),
      0
    );
}
 ...
 */
function getSafeRequiredQuantity(requiredQuantity: number): number | null {
...
function buildUnavailableAllocation(
  productId: string,
...
function allocateForItem(
  ...
  const finalStatus: AllocationStatus =
    totalAllocated >= safeRequiredQuantity ? "ALLOCATED" : "PARTIAL";
...
export function allocateWarehouseInventory(
  ...
  const activeWarehouses = safeWarehouses.filter(
