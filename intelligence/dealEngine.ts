...
/**
 * Public result returned by the DealFlow360 intelligence layer.
 *
 * This is intentionally kept stable because the backend/API layer
 * consumes this shape.
 */
...
function isFulfillmentComplete(
  ...
  for (const item of items) {
    ...
    const currentRequired =
      requiredByProduct.get(productId) ?? 0;

    requiredByProduct.set(
      productId,
      currentRequired + quantity
    );
  }
   ...
   * incomplete.
   */
  const incompleteByProduct = new Set<string>();
  ...
  for (const allocation of allocations) {
    ...
    const currentAllocated =
      allocatedByProduct.get(productId) ?? 0;

    allocatedByProduct.set(
      productId,
      currentAllocated + quantity
    );
  }
