...
/**
 * Public contract — unchanged from the pre-Phase-3 engine. `required` and
 * `reason` keep their existing names and meaning. When more than one
 * approval trigger fires, `reason` concatenates each trigger's sentence (in
 * a fixed, deterministic order) rather than dropping information.
 */
...
type KnownRiskLevel = (typeof KNOWN_RISK_LEVELS)[number];

function isKnownRiskLevel(value: unknown): value is KnownRiskLevel {
  return (
    typeof value === "string" &&
...
function isHighOrCriticalRisk(riskLevel: KnownRiskLevel): boolean {
 ...
 */
function evaluateDiscountTrigger(
  discountEvaluation: DiscountEvaluation | null | undefined
): Trigger | null {
  if (!discountEvaluation) {
    return {
      reason:
        "Discount evaluation is unavailable, so approval is required as a precaution.",
    };
  }
  ...
  if (typeof discountEvaluation.exceeded !== "boolean") {
    return {
      reason:
        "Discount evaluation result is incomplete, so approval is required as a precaution.",
    };
  }
...
function evaluateRiskTrigger(
  riskEvaluation: RiskEvaluation | null | undefined
): Trigger | null {
  if (!riskEvaluation) {
    return {
      reason:
        "Risk evaluation is unavailable, so approval is required as a precaution.",
    };
  }
  ...
  if (!isKnownRiskLevel(riskEvaluation.riskLevel)) {
    return {
      reason:
        "Deal risk level could not be determined, so approval is required as a precaution.",
    };
  }
 ...
 */
function evaluateMarginTrigger(
  ...
  if (!Number.isFinite(marginPercent)) {
    return {
      reason:
        "Deal margin could not be determined, so approval is required as a precaution.",
    };
  }
